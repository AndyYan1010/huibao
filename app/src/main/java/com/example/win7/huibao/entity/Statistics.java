package com.example.win7.huibao.entity;

public class Statistics {
    String fbillno;
    String qi;
    String zhi;
    String zuzhi;
    String zeren;
    String wanglai;
    String jihua;
    String neirong;
    String shuliang;
    String danjia;
    String hanshui;
    String buhan;

    public String getFbillno() {
        return fbillno;
    }

    public void setFbillno(String fbillno) {
        this.fbillno = fbillno;
    }

    public String getQi() {
        return qi;
    }

    public void setQi(String qi) {
        this.qi = qi;
    }

    public String getZhi() {
        return zhi;
    }

    public void setZhi(String zhi) {
        this.zhi = zhi;
    }

    public String getZuzhi() {
        return zuzhi;
    }

    public void setZuzhi(String zuzhi) {
        this.zuzhi = zuzhi;
    }

    public String getZeren() {
        return zeren;
    }

    public void setZeren(String zeren) {
        this.zeren = zeren;
    }

    public String getWanglai() {
        return wanglai;
    }

    public void setWanglai(String wanglai) {
        this.wanglai = wanglai;
    }

    public String getJihua() {
        return jihua;
    }

    public void setJihua(String jihua) {
        this.jihua = jihua;
    }

    public String getNeirong() {
        return neirong;
    }

    public void setNeirong(String neirong) {
        this.neirong = neirong;
    }

    public String getShuliang() {
        return shuliang;
    }

    public void setShuliang(String shuliang) {
        this.shuliang = shuliang;
    }

    public String getDanjia() {
        return danjia;
    }

    public void setDanjia(String danjia) {
        this.danjia = danjia;
    }

    public String getHanshui() {
        return hanshui;
    }

    public void setHanshui(String hanshui) {
        this.hanshui = hanshui;
    }

    public String getBuhan() {
        return buhan;
    }

    public void setBuhan(String buhan) {
        this.buhan = buhan;
    }
}
